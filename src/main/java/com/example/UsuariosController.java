...
public class UsuariosController {
    ...
    private HBox criarCardAdministrador(Adm adm) {
        ...
        excluir.setOnAction(e -> {
            if (confirmarExclusao("administrador " + adm.getFullName())) {
                boolean sucesso = AdmDAO.delete(adm.getId());
                if (sucesso) {
                    carregarDados();
                    // ✅ Notificação Telegram
                    TelegramNotifier.send(
                            "🗑️ Administrador excluído:\n👤 " + adm.getFullName() +
                                    "\n📧 " + adm.getEmail()
                    );
                }
            }
    ...
    private HBox criarCardFuncionario(Employee emp) {
        ...
        excluir.setOnAction(e -> {
            if (confirmarExclusao("funcionário " + emp.getName())) {
                boolean sucesso = EmployeeDAO.delete(emp.getId());
                if (sucesso) {
                    carregarDados();
                    // ✅ Notificação Telegram
                    TelegramNotifier.send(
                            "🗑️ Funcionário excluído:\n👤 " + emp.getName() +
                                    "\n💼 " + emp.getPosition()
                    );
                }
            }
    ...
    public void novoUsuario() {
        try {
            ...
            // ✅ Notificação Telegram (novo usuário)
            TelegramNotifier.send("👤 Novo usuário cadastrado!");
    ...
    private void editarAdministrador(Adm adm) {
        try {
            ...
            controller.setCallback(() -> {
                carregarDados();
                // ✅ Notificação Telegram (edição)
                TelegramNotifier.send("✏️ Administrador editado:\n👤 " + adm.getFullName());
            });
    ...
    private void editarFuncionario(Employee emp) {
        try {
            ...
            controller.setCallback(() -> {
                carregarDados();
                // ✅ Notificação Telegram (edição)
                TelegramNotifier.send("✏️ Funcionário editado:\n👤 " + emp.getName());
            });
